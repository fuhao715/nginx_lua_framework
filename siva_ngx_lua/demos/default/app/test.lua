...
function ltp(req, resp)
    resp:ltp("ltp.html", {v="hello, letv_ngx_lua!"})
end
...
function params2ClientInfo(req)
    local clientInfo = {}
    clientInfo['appkey']= req.uri_args["appkey"]  
    clientInfo['appversion']= req.uri_args["appversion"]  
    clientInfo['macaddr']= req.uri_args["macaddr"]  
    clientInfo['ip']= req.remote_addr
    clientInfo['devmodel']= req.uri_args["devmodel"]  
    clientInfo['devid']= req.uri_args["devid"]  
    return clientInfo
end
function upgrade(req,resp)
    logger:i("-----upgrade1111111111111111----")
    local clientInfo = params2ClientInfo(req)
                     ...
                     result = default_rt:data_not_found()
                      ...
                      if 0 == table.getn(verHash_t) then
                          ...
                          --[[
                              过滤例外
                          --]]
                          local versionRuleService = require('versionRuleService')
                          local throughResult = versionRuleService.throughUpgradeByRule(clientInfo,verId) 
