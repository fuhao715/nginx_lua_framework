...
module('siva.debug',package.seeall)

local sivautil    = require('siva.util')
local front      = require('siva.front')
...
function debug_utils()
    ...
    function _debug_hook(event, extra)
        ...
        --if (string.find(info.short_src,"siva_ngx_lua/luasrc") or 
        --string.find(info.short_src,"siva_ngx_lua/lualib")) then
        --return
        --end
        info.event=event
...
function debug_info2html()
    ...
    local ret = front.DEBUG_INFO_CSS .. [==[
                <div id="siva_ngx_lua-table-of-contents">
                <h2>DEBUG INFO </h2>
                <div id="siva_ngx_lua-text-table-of-contents"><ul>
        ]==]
