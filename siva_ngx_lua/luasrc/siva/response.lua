...
module('siva.response',package.seeall)

local sivautil    = require('siva.util')
local sivavars    = require('siva.vars')
local sivadebug   = require("siva.debug")
...
function Response:write(content)
    if self._eof==true then
        local error_info = "SIVA WARNING: The response has been explicitly finished before."
        logger:w(error_info)
...
function Response:set_cookie(key, value, encrypt, duration, path)
    ...
    ngx.header["Set-Cookie"]=siva.functional.table_values(self._cookies)
end

function Response:debug()
    local debug_conf = sivautil.get_config("debug")
    ...
    if target=="response" and string_match(self.headers['Content-Type'] or '', '^text/.*html') then
        ...
        self:write(sivadebug.debug_info2html())
    elseif target=="ngx.log" then
        ngx.log(ngx.DEBUG, sivadebug.debug_info2text())
    end
    sivadebug.debug_clear()
end

function Response:error(info)
    local error_info = "SIVA ERROR: " .. info
...
function ltp_function(template)
    ...
    local tdata=sivautil.read_all(ngx.var.SIVA_NGX_LUA_APP_PATH .. "/templates/" .. template)
    -- find subapps' templates
    if not tdata then
        tdata=(function(appname)
                   subapps=sivavars.get(appname,"APP_CONFIG").subapps or {}
                   for k,v in pairs(subapps) do
                       d=sivautil.read_all(v.path .. "/templates/" .. template)
                       if d then return d end
                   end
               end)(ngx.ctx.SIVA_NGX_LUA_APP_NAME)
    end
