...
local sivavars = require("siva.vars")

module('siva.util', package.seeall)
...
function setup_app_env(siva_home, app_name, app_path, global)
    global['SIVA_NGX_LUA_HOME']=siva_home
    global['SIVA_NGX_LUA_APP']=app_name
    global['SIVA_NGX_LUA_APP_PATH']=app_path
    package.path = siva_home .. '/lualibs/?.lua;' .. package.path
    package.path = app_path .. '/app/?.lua;' .. package.path
    local request=require("siva.request")
    local response=require("siva.response")
    global['SIVA_NGX_LUA_MODULES']={}
    global['SIVA_NGX_LUA_MODULES']['request']=request
    global['SIVA_NGX_LUA_MODULES']['response']=response
end
...
function get_config(key, default)
    ...
    if not issub then -- main app
        ...
        local app_conf=sivavars.get(ngx.ctx.SIVA_NGX_LUA_APP_NAME,"APP_CONFIG")
    ...
    local subapps=sivavars.get(ngx.ctx.SIVA_NGX_LUA_APP_NAME,"APP_CONFIG").subapps or {}
...
function is_subapp(__call_frame_level)
    ...
    local main_app = ngx.var.SIVA_NGX_LUA_APP_PATH
    ...
    return false, nil -- not main/sub app, maybe call in siva_ngx_lua!
end
