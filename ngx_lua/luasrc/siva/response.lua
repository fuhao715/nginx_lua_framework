...
module('fuhao.response',package.seeall)

local fuhaoutil    = require('fuhao.util')
local fuhaovars    = require('fuhao.vars')
local fuhaodebug   = require("fuhao.debug")
local functional = require('fuhao.functional')
...
function Response:set_cookie(key, value, encrypt, duration, path)
    ...
    ngx.header["Set-Cookie"]=fuhao.functional.table_values(self._cookies)
end

function Response:debug()
    local debug_conf = fuhaoutil.get_config("debug")
    ...
    if target=="response" and string_match(self.headers['Content-Type'] or '', '^text/.*html') then
        ...
        self:write(fuhaodebug.debug_info2html())
    elseif target=="ngx.log" then
        ngx.log(ngx.DEBUG, fuhaodebug.debug_info2text())
    end
    fuhaodebug.debug_clear()
end
...
function ltp_function(template)
    ...
    local tdata=fuhaoutil.read_all(ngx.var.FUHAO_NGX_LUA_APP_PATH .. "/templates/" .. template)
    -- find subapps' templates
    if not tdata then
        tdata=(function(appname)
                   subapps=fuhaovars.get(appname,"APP_CONFIG").subapps or {}
                   for k,v in pairs(subapps) do
                       d=fuhaoutil.read_all(v.path .. "/templates/" .. template)
                       if d then return d end
                   end
               end)(ngx.ctx.FUHAO_NGX_LUA_APP_NAME)
    end
