...
module('fuhao.debug',package.seeall)

local fuhaoutil    = require('fuhao.util')
local front      = require('fuhao.front')
...
function debug_utils()
    ...
    function _debug_hook(event, extra)
        ...
        --if (string.find(info.short_src,"fuhao_ngx_lua/luasrc") or 
        --string.find(info.short_src,"fuhao_ngx_lua/lualib")) then
        --return
        --end
        info.event=event
...
function debug_info2html()
    ...
    local ret = front.DEBUG_INFO_CSS .. [==[
                <div id="fuhao_ngx_lua-table-of-contents">
                <h2>DEBUG INFO </h2>
                <div id="fuhao_ngx_lua-text-table-of-contents"><ul>
        ]==]
