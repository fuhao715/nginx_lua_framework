...
module("fuhao.logger", package.seeall)
...
fuhaoutil = require("fuhao.util")
fuhaovars = require("fuhao.vars")

function get_logger(appname)
    local logger = fuhaovars.get(appname, "__LOGGER")
    if logger then return logger end
    
    local filename = "/dev/stderr"
    local level = "DEBUG"
    -- local log_config = fuhaoutil.get_config(appname, "logger")
    local log_config = fuhaoutil.get_config("logger")
    ...
    fuhaovars.set(appname, "__LOGGER", logger)
    logger._log = logger.log
    logger._setLevel = logger.setLevel

    logger.log = function(self, level, ...)
                     local _logger = get_logger(ngx.ctx.FUHAO_NGX_LUA_APP_NAME)
    ...
    logger.table_print = fuhaoutil.table_print
    
    return logger
end

function logger()
    local logger = get_logger(ngx.ctx.FUHAO_NGX_LUA_APP_NAME)
    return logger
end
